...
def epoch_pass(
    ...
    for batch_idx, batch in enumerate(tqdm(loader)): 
        if train:
            ...
            pred_action, pred_action_v, pred_obj, losses = model(batch, epoch=epoch, train=train)     # handformer용
            loss = losses["l1_loss"]
            # wandb.log({
            #     "train/total_loss": loss.mean().item() if loss.numel() > 1 else loss.item(),
            #     "train/olabel_loss": losses.get("olabel_loss", torch.tensor(0.0)).mean().item(),
            #     "train/hlabel_loss": losses.get("hlabel_loss", torch.tensor(0.0)).mean().item(),
            #     "train/action_loss": losses.get("action_loss", torch.tensor(0.0)).mean().item(),
            ...
            #     "val/olabel_loss": losses.get("olabel_loss", torch.tensor(0.0)).item(),
            #     "val/hlabel_loss": losses.get("hlabel_loss", torch.tensor(0.0)).item(),
            #     "val/action_loss": losses.get("action_loss", torch.tensor(0.0)).item(),
            #     "epoch": epoch
            # })
        ...
        if train:
            ...
            loss.backward()  
            for name, param in (model.module if use_multiple_gpu else model).named_parameters():
                if param.requires_grad and "classifier_action" in name:
                    print(f"[Grad Norm] {name}:", param.grad.norm().item() if param.grad is not None else "NO GRAD")
        ...
        if not train:
            ...
            # # Object
            # results["obj_pred_labels"] = (pred_obj.sigmoid() > 0.5).float()
            # results["obj_gt_labels"] = torch.zeros_like(pred_obj)
            # for i, obj_ids in enumerate(batch["obj_idx"]):
            #     for obj_id in obj_ids:
            #         results["obj_gt_labels"][i, obj_id] = 1.0
