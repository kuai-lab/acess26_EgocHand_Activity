...
class FrameClassificationEvaluator:
    ...
    def feed(self, gt_labels, pred_labels, weights, verbose=False):   
        ...
        for idx in range(gt_labels.shape[0]):
            if weights[idx] > 1e-4:
                gt_set = gt_labels[idx]
                pred_set = pred_labels[idx]

                # 리스트나 벡터 형태를 대응하도록 변환
                if isinstance(gt_set, (int, float)):
                    gt_set = [int(gt_set)]
                elif isinstance(gt_set, (list, np.ndarray)):
                    gt_set = [int(x) for x in gt_set if x >= 0]

                if isinstance(pred_set, (int, float)):
                    pred_set = [int(pred_set)]
                elif isinstance(pred_set, (list, np.ndarray)):
                    pred_set = [int(x) for x in pred_set if x >= 0]

                # GT-PRED 조합 기록
                for gt in gt_set:
                    for pred in pred_set:
                        self.count_matrix[gt, pred] += 1
    ...
    def get_recall_rate(self, aggregate_sequence=True, verbose=False):        
        ...
        if aggregate_sequence:
            for k,v in self.video_seq_results.items():
                print("Key:", k, "→ gt_id:", k[-1], "→ pred_id:", np.argmax(v))
                gt_id=k[-1]
                pred_id=np.argmax(v)
