...
class SeqSet(Dataset): 
    ...
    def get_sample(self, idx, query=None, seq_txn=None, color_augm=None, space_augm=None):
        ...
        #Get mediapipe keypoint --> gibson
        keypoints = self.pose_dataset.sample_infos[idx].get("keypoints", None)
        sample[BaseQueries.HANDKEYPOINTS] = keypoints.astype(np.float32)
    ...
    def __getitem__(self, idx, verbose=False):
        ...
        for sample_idx in range(self.ntokens - 1):
            while True:
                ...
                sample_fut_frame = self.get_safesample(
                    fut_idx, seq_txn=seq_txn, color_augm=color_augm, space_augm=space_augm
                )
                sample_fut_frame["dist2query"] = fut_idx - frame_idx
                sample_fut_frame["not_padding"] = fut_not_padding
                # :white_check_mark: 액션 인덱스를 복사
                if BaseQueries.ACTIONIDX in sample:
                    sample_fut_frame[BaseQueries.ACTIONIDX] = sample[BaseQueries.ACTIONIDX]
                # remove augmentation records
                sample_fut_frame.pop("space_augm")
                sample_fut_frame.pop("color_augm")
                if fut_not_padding != 0:
                    break
                else:
                    cur_idx = fut_idx  # skip and retry next future frame
