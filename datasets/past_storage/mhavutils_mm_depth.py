...
# multi-modal(depth, ) 용도로 변경
import os
import pandas as pd

def get_all_hand_labels(file_path, rgb_root, rgb_template, depth_root, depth_template): 
    info = dict()
    subjects_infos = {}
    ...
    for idx, (name, l_info, r_info, object) in enumerate(zip(df['DATA_MHAV'], df.L, df.R, df.object)):
        ...
        if save:
            ...
            # thermal_dir = os.path.join(thermal_root, base_dir, "overlay", "processed_270_480")
                ...
                if abs(last_frame_idx - l_max) <= 1 and abs(last_frame_idx - r_max) <= 1:
                    for frame_idx in range(len(frames)):
                        ...
                        if not (os.path.exists(rgb_path) and os.path.exists(depth_path) ):
                            save = False
                            reason = f"[MISSING FILE] RGB or Depth  missing at frame {frame_idx}"
                            break
                else:
                    save = False
