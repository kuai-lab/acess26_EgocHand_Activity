...
from datasets import mhavutils_depth
...
class MHAVhands(object):
    def __init__(
        ...
        # if os.path.exists(small_depth) and self.reduce_res: 
        #     self.depth_root = small_depth
        #     self.reduce_factor = 1 / 4
        # else:
        #     self.depth_root = self.root, "Video_files"
        ...
        small_depth = self.root  # 또는 실제 리사이즈 이미지 경로
        ...
        if os.path.exists(small_depth) and self.reduce_res:
            ...
            self.depth_root = os.path.join(self.root, "Video_files")  # 🔧 수정
            self.reduce_factor = 1
        ...
        action_info, action_to_idx = mhavutils_depth.get_action_infos(path_action_info)
        ...
        object_info, object_to_idx = mhavutils_depth.get_object_infos(path_object_info)
        ...
        self.hand_labels, self.subjects_infos = mhavutils_depth.get_all_hand_labels('../data_MHAV/annotation_0317.xlsx', self.depth_root, self.depth_template)
    ...
    #             #     # relative_img_path = os.path.join(action_name, subject, name, "depth_undistorted", self.depth_template.format(iidx))
    #             #     relative_img_path = os.path.join(action_name, subject, name, "depth_undistorted", "processed_270_480", self.depth_template.format(iidx))
    ...
    #                     "../data_MHAV", action_name, subject, name, "depth_undistorted", "processed_270_480", self.depth_template.format(iidx)
    #                 )
    ...
    #     window_starts,fulls= mhavutils_depth.get_seq_map(sample_infos=self.sample_infos,video_lens=self.video_lens,
    ...
    def load_dataset(self):
            ...
            for name in self.subjects_infos[sub]:
                ...
                for iidx in range(int(frame_idx)):
                    relative_img_path = os.path.join(
                        ...
                        "Depth_new", "processed_270_480",
                        self.depth_template.format(iidx)
                    )
        ...
        window_starts, fulls = mhavutils_depth.get_seq_map(
    ...
    def get_image(self, idx, txn=None):
        ...
        img_path = os.path.join(self.depth_root, img_path)
