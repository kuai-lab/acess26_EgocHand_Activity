...
from datasets import mhavutils_mm_new_thermal_final
...
class MHAVhands(object):
    def __init__(
        ...
        self.thermal_template = "warped_thermal_{:04d}.jpg"
        self.mask_template = "Thermal_threashold_{}.jpg"
        ...
        action_info, action_to_idx = mhavutils_mm_new_thermal_final.get_action_infos(path_action_info)
        ...
        object_info, object_to_idx = mhavutils_mm_new_thermal_final.get_object_infos(path_object_info)
        ...
        self.hand_labels, self.subjects_infos = mhavutils_mm_new_thermal_final.get_all_hand_labels('../data_MHAV/annotation_0317.xlsx', self.rgb_root, self.rgb_template, self.thermal_root, self.thermal_template)
    ...
    def load_dataset(self):
            ...
            for name in self.subjects_infos[sub]:
                ...
                for iidx in range(int(frame_idx)):
                    ...
                    modal_paths.append({
                        ...
                        "thermal": os.path.join(base_dir, "warped_thermal", "processed_270_480", self.thermal_template.format(iidx)),
                    })
                    relative_mask_path = os.path.join(base_dir, "Thermal_threashold", "processed_270_480", self.mask_template.format(iidx)),
                    sample_infos.append({
                        "subject": subject,
        ...
        window_starts, fulls = mhavutils_mm_new_thermal_final.get_seq_map(
    ...
    def get_modal_images(self, idx, txn = None):
        ...
        # thermal_thres = Image.open(os.path.join(self.thermal2_root, paths["thermal2"])).convert("RGB")
        # print(f"[DEBUG] Thermal path: {thermal_path}")
        return rgb, thermal


    def load_threshold_mask(self, mask_path, target_shape):
        img = Image.open(mask_path).convert("RGB").resize(target_shape[::-1])  # (W,H)
        mask_np = np.array(img)
        binary_mask = (mask_np[:, :, 0] > 128).astype(np.float32)  # Red channel
        return torch.tensor(binary_mask).unsqueeze(0)  # [1,H,W]
    ...
    def __getitem__(self, idx):
        ...
        rgb_img, thermal_img = self.get_modal_images(idx)
        mask_path = self.mask_names[idx]
        W, H = rgb_img.size
        mask_tensor = self.load_threshold_mask(mask_path, target_shape=(H, W))

        action_idx = self.get_action_idxs(idx)
        obj_idx = self.get_obj_idxs(idx)

        sample = {
            TransQueries.IMAGE: (rgb_img, thermal_img),
            TransQueries.ACTIONIDX: action_idx,
            TransQueries.OBJIDX: obj_idx,
            TransQueries.CAMINTR: torch.tensor(self.cam_intr, dtype=torch.float32),
            TransQueries.MASK: mask_tensor
        }
