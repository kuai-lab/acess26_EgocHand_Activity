...
from datasets import mhavutils_mm_dup
...
class MHAVhands(object):
    def __init__(
        ...
        self.rgb2_template = "RGB_undistorted_{}.jpg"
        self.rgb3_template = "RGB_undistorted_{}.jpg"

        print("앙 확인띠", self.root)
        small_rgb = self.root  # 또는 실제 리사이즈 이미지 경로
        small_rgb2 = self.root
        small_rgb3 = self.root

        if os.path.exists(small_rgb) and self.reduce_res:
            self.rgb_root = small_rgb
            self.rgb2_root = small_rgb
            self.rgb3_root = small_rgb
        ...
        action_info, action_to_idx = mhavutils_mm_dup.get_action_infos(path_action_info)
        ...
        object_info, object_to_idx = mhavutils_mm_dup.get_object_infos(path_object_info)
        ...
        self.hand_labels, self.subjects_infos = mhavutils_mm_dup.get_all_hand_labels('../data_MHAV/annotation_0317.xlsx', self.rgb_root, self.rgb_template,  self.rgb2_root, self.rgb2_template, self.rgb3_root, self.rgb3_template)
    ...
    def load_dataset(self):
            ...
            for name in self.subjects_infos[sub]:
                ...
                for iidx in range(int(frame_idx)):
                    ...
                    modal_paths.append({
                        ...
                        "rgb2": os.path.join(base_dir, "RGB_undistorted", "processed_270_480", self.rgb_template.format(iidx)),
                        "rgb3": os.path.join(base_dir, "RGB_undistorted", "processed_270_480", self.rgb_template.format(iidx)),
                    })
        ...
        window_starts, fulls = mhavutils_mm_dup.get_seq_map(
    ...
    def get_modal_images(self, idx, txn = None):
        ...
        rgb2 = Image.open(os.path.join(self.rgb_root, paths["rgb"])).convert("RGB")
        rgb3 = Image.open(os.path.join(self.rgb_root, paths["rgb"])).convert("RGB")
