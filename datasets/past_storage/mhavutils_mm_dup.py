...
def get_all_hand_labels(file_path, rgb_root, rgb_template, rgb2_root, rgb2_template, rgb3_root, rgb3_template): 
    info = dict()
    subjects_infos = {}
    ...
    for idx, (name, l_info, r_info, object) in enumerate(zip(df['DATA_MHAV'], df.L, df.R, df.object)):
        ...
        if save:
            ...
            rgb2_dir = os.path.join(rgb_root, base_dir, "RGB_undistorted", "processed_270_480")
            rgb3_dir = os.path.join(rgb_root, base_dir, "RGB_undistorted", "processed_270_480")
            ...
            if os.path.exists(rgb_dir):
                ...
                if abs(last_frame_idx - l_max) <= 1 and abs(last_frame_idx - r_max) <= 1:
                    for frame_idx in range(len(frames)):
                        ...
                        rgb2_path = os.path.join(rgb_dir, rgb_template.format(frame_idx))
                        rgb3_path = os.path.join(rgb_dir, rgb_template.format(frame_idx))

                        if not (os.path.exists(rgb_path) and os.path.exists(rgb2_path) and os.path.exists(rgb3_path)):
                            save = False
                            reason = f"[MISSING FILE] RGB or RGB2 or RGB3 missing at frame {frame_idx}"
                            break
                else:
                    save = False
