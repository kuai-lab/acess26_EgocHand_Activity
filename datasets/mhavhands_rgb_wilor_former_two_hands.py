...
from datasets import mhavutils_rgb_wilor
...
class MHAVhands(object):
    def __init__(
        ...
        action_info, action_to_idx = mhavutils_rgb_wilor.get_action_infos(path_action_info)
        ...
        object_info, object_to_idx = mhavutils_rgb_wilor.get_object_infos(path_object_info)
        ...
        self.hand_labels, self.subjects_infos = mhavutils_rgb_wilor.get_all_hand_labels('../data_MHAV/annotation_0620_final.xlsx', self.rgb_root, self.rgb_template, self.thermal_root, self.mp_template)
    ...
    def get_pose_seq(self, idx):
        """
        주어진 window index (self.window_starts 기준)에서 sliding window 시퀀스 pose_2d CSV들을 로드해 하나의 텐서로 반환.
        리턴 shape: [T, J, 3]  (T=ntokens_action, J=21 or 42, 3=(x,y,z))
        """
        start_idx = self.get_start_frame_idx(idx)
        pose_seq = []

        for t in range(self.ntokens_action):
            frame_idx = start_idx + t * self.spacing
            if frame_idx >= len(self.modal_paths):
                break  # 마지막 시퀀스는 짧을 수 있음

            pose_csv_path = os.path.join(self.rgb_root, self.modal_paths[frame_idx]["mp"])
            try:
                pose_np = np.loadtxt(pose_csv_path, delimiter=",", skiprows=1)  # shape: [J, 3]
            except:
                pose_np = np.zeros((42, 3))  # 비었거나 오류일 경우 fallback

            pose_seq.append(pose_np)

        pose_seq = np.stack(pose_seq, axis=0)  # shape: [T, J, 3]
        return torch.tensor(pose_seq, dtype=torch.float32)
    ...
    def load_dataset(self):
            ...
            for name in self.subjects_infos[sub]:
                ...
                for iidx in range(int(frame_idx)):
                    ...
                    if os.path.exists(mp_path):
                        # CSV 로드 시 실제 값이 모두 0이 아닌지 체크 !!!! 
                        ## ==== 두 손의 포즈가 탐지된 csv만 로드하게 하는 버전 ==== ##
                        keypoints = np.loadtxt(mp_path, delimiter=",", skiprows=1)
                        if keypoints.shape == (42, 3):
                            if not np.allclose(keypoints, 0):  # 42x3 전체가 0인지 검사
                                final_mp_path = mp_path
                            else:
                                continue  # 0인 경우 샘플 스킵
                        else:
                            continue  # 42줄이 아니면 스킵
                    else:
                        continue  # CSV 자체가 없으면 스킵
                    ...
                    if action_name in ["assemble", "screw", "clamp"]:
                        bidir_label = 0
                    elif action_name in ["unassemble", "unscrew", "unclamp"]:
                        bidir_label = 1
                    else:
                        bidir_label = -1
        ...
        window_starts, fulls = mhavutils_rgb_wilor.get_seq_map(
