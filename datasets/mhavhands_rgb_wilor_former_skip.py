...
class MHAVhands(object):
    def __init__(
        ...
        for i,(k,v) in enumerate(self.action_info.items()):
         ...
         # Load skip list
        skip_path = '/data/jyseo/data_MHAV/screw-unscres-skip-final_0616.xlsx'
        self.skip_seqs = set()
        if os.path.exists(skip_path):
            # df = pd.read_excel(skip_path, header=None, engine="openpyxl")
            if skip_path.endswith(".xlsx"):
                df = pd.read_excel(skip_path, engine="openpyxl")
            elif skip_path.endswith(".ods"):
                df = pd.read_excel(skip_path, engine="odf")
            else:
                raise ValueError(f"Unsupported file format: {skip_path}")

            skip_values = df.iloc[0:, 0].dropna().astype(str).tolist()  # 2행부터, 첫 번째 열
