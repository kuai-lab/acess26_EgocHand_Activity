...
from models.htt_mm_cma import TemporalNet
from netscripts import epochpass
from netscripts import reloadmodel, get_dataset_mm_dup 
from torch.utils.tensorboard import SummaryWriter
from netscripts.get_dataset_mm_dup import DataLoaderX 
from datasets.queries import TransQueries
...
def collate_fn(seq, extend_queries=None):
    if extend_queries is None:
        extend_queries = [TransQueries.IMAGE, TransQueries.DEPTH, TransQueries.THERMAL]
...
def main(args):
    ...
    train_dataset, _ = get_dataset_mm_dup.get_dataset_htt(
        args.train_dataset,
    ...
    print("MHAV train dataset(multi_modal) loading...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--experiment_tag',default='cma') 
    parser.add_argument('--dataset_folder',default='../data_MHAV/')  # 바꿔야댐
    parser.add_argument('--cache_folder',default='./jy/ckpts/')     # 없음
    parser.add_argument('--resume_path',default='./jy/ckpts/cma/checkpoint_1.pth')     
    ...
    #Loss
    parser.add_argument("--lambda_action_loss",type=float, default=2, help="Weight for action/object classification")#lambda for action, lambda_3
