...
# 추가 (Gated Cross Modality Attention) 
# GatedCrossModalAttention -> CrossGatedInteraction으로 확장
import torch
import torch.nn as nn
...
class CrossGatingBlock(nn.Module):
    ...
    def forward(self, H, C):
        # H: 원래 modality feature [B, 1, D]
        # C: 다른 modality에서 cross-attn으로 얻은 context [B, 1, D]
        G = self.sigmoid_gate(self.proj_context(C))  # gate 값 생성
        return H * G + C * (1 - G)  # gated fusion


class CrossGatedCrossModalAttention(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8):
        super().__init__()
        self.attn = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True)
        self.norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(0.1)
        self.gate = CrossGatingBlock(embed_dim)

    def forward(self, q, kv):
        # Cross Attention
        attn_out, _ = self.attn(q, kv, kv)  # context from other modalities

        # Cross Gating
        gated = self.gate(q, attn_out)  # (H_q, F_attn)

        # Residual + Norm
        return self.norm(q + self.dropout(gated))


#  (Bidirectional Cross Gated Attention)
class BidirectionalCrossGatedCMA(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8):
        super().__init__()
        self.rgb_to_others = CrossGatedCrossModalAttention(embed_dim, num_heads)
        self.depth_to_others = CrossGatedCrossModalAttention(embed_dim, num_heads)
        self.thermal_to_others = CrossGatedCrossModalAttention(embed_dim, num_heads)
        self.fuse = nn.Linear(embed_dim * 3, embed_dim)

    def forward(self, rgb, depth, thermal):
        rgb_feat = self.rgb_to_others(rgb, torch.cat([depth, thermal], dim=1))
        depth_feat = self.depth_to_others(depth, torch.cat([rgb, thermal], dim=1))
        thermal_feat = self.thermal_to_others(thermal, torch.cat([rgb, depth], dim=1))
        return self.fuse(torch.cat([rgb_feat, depth_feat, thermal_feat], dim=-1))
...
class TemporalNet(torch.nn.Module):
    def __init__(self,  is_single_hand,
        ...
        self.reduce_feature = torch.nn.Linear(512 * 3, transformer_d_model)

        # Feature Extraction
        # self.meshregnet = ResNet_(resnet_version=18)

        # Feature Extractors for each modality
        self.rgb_resnet = ResNet_(resnet_version=18)
        self.depth_resnet = ResNet_(resnet_version=18)
        self.thermal_resnet = ResNet_(resnet_version=18)

        self.cross_modal_attn = BidirectionalCrossGatedCMA(embed_dim=512)
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        # ⬇️ 1. Get each modality input
        rgb_input, depth_input, thermal_input = batch_flatten[TransQueries.IMAGE]
        rgb_input = rgb_input.cuda()
        depth_input = depth_input.cuda()
        thermal_input = thermal_input.cuda()

        # ⬇️ 2. Pass through each ResNet separately
        # ======== Feature Extraction ========
        rgb_feat, _     = self.rgb_resnet(rgb_input)        # [B, 512]
        depth_feat, _   = self.depth_resnet(depth_input)    # [B, 512]
        thermal_feat, _ = self.thermal_resnet(thermal_input)# [B, 512]

        rgb_feat = rgb_feat.unsqueeze(1)         # [B, 1, 512]
        depth_feat = depth_feat.unsqueeze(1)
        thermal_feat = thermal_feat.unsqueeze(1)

        # Cross Attention (Query = RGB <= Key,Value = Depth + thermal)
        # (1) Gated-CMA output 확인
        fused_feat = self.cross_modal_attn(rgb_feat, depth_feat, thermal_feat)

        if fused_feat.ndim == 3:
            fused_feat = fused_feat.squeeze(1)  #  Ensure [B, D]
        # print(fused_feat.shape)

        # Normalization
        fused_feat = nn.LayerNorm(fused_feat.shape[-1]).to(fused_feat.device)(fused_feat)
        flatten_in_feature = fused_feat

        # Loss
        total_loss = torch.Tensor([0]).cuda()
        # total_loss = torch.tensor(0.0, device=flatten_in_feature.device, requires_grad=True)
        losses = {}
        results = {}
        ...
        # flatten_ain_feature = flatten_ain_feature_olabel  # 단일 텐서니까 cat 불필요
        ...
        # ✨ 여기서 안전하게 batch 크기 맞춰주기
        B_tokens, D = flatten_ain_feature.shape
        ntokens = self.ntokens_action


        # ❗ view 전에 shape 검증
        if B_tokens % ntokens != 0:
            if verbose:
                print(f"[Warning] Truncating {B_tokens % ntokens} tokens for ntokens_action alignment.")
            flatten_ain_feature = flatten_ain_feature[: (B_tokens // ntokens) * ntokens]

        B = flatten_ain_feature.shape[0] // ntokens
        batch_seq_ain_feature = flatten_ain_feature.contiguous().view(B, ntokens, D)
        ...
        flat_padding = batch_flatten['not_padding'].cuda().float()
        padding_len = flat_padding.shape[0]
        tokens_per_seq = self.ntokens_action

        # ✨ View 전에 길이 체크해서 맞춰주기
        if padding_len % tokens_per_seq != 0:
            if verbose:
                print(f"[Warning] Padding truncated from {padding_len} → {padding_len // tokens_per_seq * tokens_per_seq}")
            flat_padding = flat_padding[: (padding_len // tokens_per_seq) * tokens_per_seq]

        batch_seq_weights_action = flat_padding.view(-1, tokens_per_seq)
    ...
    def predict_object(self, sample, features, weights, total_loss, verbose=False):
        ...
        for i, (feat_row, obj_ids) in enumerate(zip(features, obj_idx_list)):
            for obj_id in obj_ids:
                if obj_id < num_classes:
                    olabel_gts[i, obj_id] = 1.0
                else:
                    print(f"[Warning] Skipping obj_id={obj_id} (out of bounds), num_classes={num_classes}")
