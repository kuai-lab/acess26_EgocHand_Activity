...
class TemporalNet(torch.nn.Module):
    def __init__(self,  is_single_hand,
        ...
        self.reduce_feature = torch.nn.Linear(1024, transformer_d_model)    # imagebind feature(1024) => 512

        # Feature Extraction

        self.imagebind = imagebind_model.imagebind_huge(pretrained=True)
        self.imagebind = self.imagebind.half()  # ← 핵심!
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        ...
        inputs = {
            ModalityType.VISION: data.load_and_transform_vision_tensor(rgb_input, self.device).half(),
            ModalityType.THERMAL: data.load_and_transform_thermal_tensor(thermal_input, self.device).half(),
                }
        # with torch.no_grad():
        with autocast(enabled=True):
            embeddings = self.imagebind(inputs)     # ImageBind embedding space로 


        # ⬇️ 2. Pass through Imagebind Encoder
        # ======== Feature Extraction ========
        fused_embedding = (embeddings[ModalityType.VISION] +
                   embeddings[ModalityType.THERMAL]) / 2        # 일단 test용으로 평균내기, 나중에 linear concat 후 projection으로 변경유무 고민

        
        # ⬇️ 4. Reduce dimension (if needed)
        # ======== Feature Extraction ========
        
        flatten_in_feature = self.reduce_feature(fused_embedding)  # [B, 512] ← transformer_d_model
