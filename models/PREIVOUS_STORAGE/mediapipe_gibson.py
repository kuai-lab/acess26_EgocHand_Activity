...
class TemporalNet(torch.nn.Module):
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        flatten_images, _=batch_flatten[TransQueries.IMAGE].cuda()
        flatten_imgs, _=batch_flatten[BaseQueries.IMAGE].cuda() #mediapipe에는 원본 이미지를 쓰자
        ...
        # ========= media_pipe handkey extract Module ======
        # B, C, H, W = flatten_imgs.shape
        # mp_keypoints = []

        # for i in range(B):
        #     img_tensor = flatten_imgs[i].detach().cpu()
        ...
        #                 # 시각화 할 때는 정수로 뽑아 봐야됌
        #                 # x_px = int(np.clip(lm.x * W_img, 0, W_img - 1))
        #                 # y_px = int(np.clip(lm.y * H_img, 0, H_img - 1))

        #                 kp[hand_idx * 21 + lm_idx] = torch.tensor([x_px, y_px])
        #                 # cv2.circle(img, (x_px,y_px), radius=2, color=(255, 0, 0), thickness=-1)
                        
        #     mp_keypoints.append(kp)
            
        #     # img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        #     # cv2.imwrite(os.path.join('./gibson/check_mp', f"sample_{i}.png"), img_bgr)

        
        # mp_keypoints_tensor = torch.stack(mp_keypoints).cuda()
        # print("mp_keypoints_tensor", mp_keypoints_tensor.shape) #torch.Size([256, 42, 2])
        
        
        #csv를 가지고 오는 ver
        # mp_keypoints_tensor = batch_flatten[BaseQueries.HANDKEYPOINTS].cuda()
        # print("mp_keypoints_tensor.shape", mp_keypoints_tensor.shape)
        
        mp_keypoints_tensor = mp_keypoints_tensor.view(mp_keypoints_tensor.shape[0], -1)  # [256, 84]
        flatten_mp_feature = self.hand_pose3d_to_action_input(mp_keypoints_tensor)  # [256, 512]
        # print("flatten_mp_feature", flatten_mp_feature.shape) #[256, 512]
        ...
        # print("1", flatten_ain_feature.shape)
        
        #Concat mediapipe handkeypoint flatten_mp_feature
        ...
        # print("2", flatten_ain_feature.shape)
    ...
    # def predict_handtype(self, sample, features, weights, total_loss, verbose=False):     # fpha(한 손)
