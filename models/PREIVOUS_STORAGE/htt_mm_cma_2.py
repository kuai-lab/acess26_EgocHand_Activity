...
class CrossModalAttention(torch.nn.Module):
    ...
    def forward(self, q, kv):
        ...
        return self.norm(q + self.dropout(out))  # residual + norm



class BidirectionalCMA(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8):
        super().__init__()
        self.rgb_to_others = CrossModalAttention(embed_dim, num_heads)
        self.depth_to_others = CrossModalAttention(embed_dim, num_heads)
        self.thermal_to_others = CrossModalAttention(embed_dim, num_heads)
...
class TemporalNet(torch.nn.Module):
    def __init__(self,  is_single_hand,
        ...
        self.cross_modal_attn = BidirectionalCMA(embed_dim=512)
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        ...
        # Key, Value Feature
        kv_feat = torch.cat([depth_feat, thermal_feat], dim=1)  # [B, 2, 512]
