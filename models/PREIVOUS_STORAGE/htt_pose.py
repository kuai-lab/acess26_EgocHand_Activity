...
class TemporalNet(torch.nn.Module):
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
       ...
       # ======== Mediapipe Inference for 2D Keypoints ========
        mp_hands = mp.solutions.hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.5)
        B = flatten_images.shape[0]
        C, H, W = flatten_images.shape[1:]

        mp_2d_keypoints = torch.zeros((B, 21, 2), dtype=torch.float32)

        for i in range(B):
            img_tensor = flatten_images[i].detach().cpu()  # (3, H, W)
            img = (img_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8)  # (H, W, 3) RGB
            img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)

            res = mp_hands.process(img)
            if res.multi_hand_landmarks:
                for lm_idx, lm in enumerate(res.multi_hand_landmarks[0].landmark):
                    x_px = int(lm.x * W)
                    y_px = int(lm.y * H)
                    mp_2d_keypoints[i, lm_idx] = torch.tensor([x_px, y_px])

        mp_hands.close()

        # Normalize keypoints to [-1, 1]
        mp_2d_keypoints[:, :, 0] = (mp_2d_keypoints[:, :, 0] / W) * 2 - 1
        mp_2d_keypoints[:, :, 1] = (mp_2d_keypoints[:, :, 1] / H) * 2 - 1
        mp_2d_flat = mp_2d_keypoints.view(B, -1).cuda()  # (B, 42)
        ...
        # # Hand Pose Estimation
        mlp_input = torch.cat([flatten_pout_feature, mp_2d_flat], dim=-1)  # (B*T, D+42)
        flatten_hpose=self.image_to_hand_pose(mlp_input)
