...
class TemporalNet(torch.nn.Module):
    def __init__(self,  is_single_hand,
        ...
        # self.reduce_feature_obj = torch.nn.Linear(1024, transformer_d_model)  # for RGB+Depth
        self.rgb_linear = torch.nn.Linear(512, transformer_d_model)
        self.depth_linear = torch.nn.Linear(512, transformer_d_model)
        self.thermal_linear = torch.nn.Linear(512, transformer_d_model)
        self.rgb_depth_obj_linear = torch.nn.Linear(512, transformer_d_model)  # obj 전용

        self.test_modalities = test_modalities if test_modalities is not None else ["rgb", "depth", "thermal"]
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        ...
        # use_depth = train or ("depth" in self.test_modalities)
        use_thermal = train or ("thermal" in self.test_modalities)

        # Selective Modality Dropout
        rgb_input = rgb_input.cuda()
        # depth_input = depth_input.cuda() if use_depth else None
        thermal_input = thermal_input.cuda() if use_thermal else None

        # ⬇️ 1. 모달별 feature 추출
        rgb_feat, _     = self.rgb_resnet(rgb_input)
        # depth_feat, _   = self.depth_resnet(depth_input)
        thermal_feat, _ = self.thermal_resnet(thermal_input)
        
        
        if train:
            flatten_in_feature = (
                self.rgb_linear(rgb_feat) +
                self.thermal_linear(thermal_feat)
            )
        else:
            flatten_in_feature = self.rgb_linear(rgb_feat)
            # if depth_input is not None:
            #     flatten_in_feature += self.depth_linear(depth_feat)
            if thermal_input is not None:
                flatten_in_feature += self.thermal_linear(thermal_feat)

        # obj 전용 (thermal 제외)
        obj_input_feat = self.rgb_linear(rgb_feat)
        ...
        # ======== Egocentric Knowledge Module (shared) ========
        batch_seq_pin_feature = flatten_in_feature.view(-1, self.ntokens_pose, flatten_in_feature.shape[-1])
        ...
        batch_seq_pweights[:, 0] = 1.0
        batch_seq_pmasks = (1 - batch_seq_pweights).bool()

        batch_seq_pout_feature, _ = self.transformer_pose(
            src=batch_seq_pin_feature,
            src_pos=batch_seq_pin_pe,
            key_padding_mask=batch_seq_pmasks,
            verbose=False
        )

        flatten_pout_feature = batch_seq_pout_feature.flatten(start_dim=0, end_dim=1)

        # ======== Egocentric Knowledge Module (thermal 제외, object 전용) ========
        obj_seq_feat = obj_input_feat.view(-1, self.ntokens_pose, obj_input_feat.shape[-1])
        obj_seq_feat_pe = self.transformer_pe(obj_seq_feat)
        obj_seq_pweights = batch_flatten['not_padding'].cuda().float().view(-1, self.ntokens_pose)
        obj_seq_pweights[:, 0] = 1.0
        obj_seq_pmasks = (1 - obj_seq_pweights).bool()

        obj_seq_pout_feature, _ = self.transformer_pose(
            src=obj_seq_feat,
            src_pos=obj_seq_feat_pe,
            key_padding_mask=obj_seq_pmasks,
            verbose=False
        )

        flatten_pout_feature_for_object = obj_seq_pout_feature.flatten(start_dim=0, end_dim=1)

        # ======== Object Classification (thermal 제외한 feature 사용) ========
        flatten_olabel_feature = self.image_to_olabel_embed(flatten_pout_feature_for_object)
