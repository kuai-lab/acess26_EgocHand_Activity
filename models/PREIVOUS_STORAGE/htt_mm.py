...
class TemporalNet(torch.nn.Module):
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        ...
        # ⬇️ 3. Concatenate
        flatten_in_feature = torch.cat([rgb_feat, depth_feat, thermal_feat], dim=-1)  # [B, 512*3]

        # ⬇️ 4. Reduce dimension (if needed)
        # ======== Feature Extraction ========

        flatten_in_feature = self.reduce_feature(flatten_in_feature)  # [B, 512] ← transformer_d_model
        ...
        batch_seq_pin_feature=flatten_in_feature.contiguous().view(-1,self.ntokens_pose,flatten_in_feature.shape[-1]) # [B//n_token, n_token, 512 ]
