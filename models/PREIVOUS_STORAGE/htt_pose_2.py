...
class TemporalNet(torch.nn.Module):
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        # mediapipe 뽑아놓은 이미지 활용하는 버전(시간절약)
        flatten_images, mp_image =batch_flatten[TransQueries.IMAGE]
        flatten_images = flatten_images.cuda()
        mp_image = mp_image.cuda()
        #Loss
        ...
        # ======== Mediapipe 키포인트 → 이미지 → Feature 추출 ========
        with torch.no_grad():
            mp_feat, _ = self.meshregnet(mp_image)  # 🔹 (B, D, H, W)일 가능성 있음

            if mp_feat.ndim == 4:
                mp_feat = torch.mean(mp_feat, dim=[2, 3])  # (B, D, H, W) → (B, D)

            mp_seq_feat = mp_feat.unsqueeze(1)  # (B, 1, D)
            T_mp = 1
            mp_seq_pe = self.transformer_pe(mp_seq_feat)
            B, C, H, W = flatten_images.shape
            mp_seq_mask = torch.zeros((B, T_mp), dtype=torch.bool).cuda()

            mp_seq_out, _ = self.transformer_pose(
                src=mp_seq_feat,
                src_pos=mp_seq_pe,
                key_padding_mask=mp_seq_mask,
                verbose=False
            )
            flatten_mp_feature = torch.flatten(mp_seq_out, start_dim=0, end_dim=1)
