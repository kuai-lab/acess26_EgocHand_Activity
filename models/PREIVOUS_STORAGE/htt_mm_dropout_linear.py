...
class TemporalNet(torch.nn.Module):
    def __init__(self,  is_single_hand,
        ...
        self.reduce_feature = torch.nn.Linear(512 * 2, transformer_d_model)
        # self.reduce_feature_obj = torch.nn.Linear(1024, transformer_d_model)  # for RGB+Depth

        self.test_modalities = test_modalities if test_modalities is not None else ["rgb", "thermal"]
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False):
        ...
        #  Shared feature for downstream tasks (train vs test 분리)
        if train:
            flatten_in_feature = self.reduce_feature(torch.cat([rgb_feat, thermal_feat], dim=-1))
        else:
            flatten_in_feature = self.reduce_feature(torch.cat(features, dim=-1))

        # object classification 전용 feature (thermal 제외)
        obj_input_feat = rgb_feat # [B, d_model]
        ...
        obj_seq_feat = obj_input_feat.contiguous().view(-1, self.ntokens_pose, obj_input_feat.shape[-1])
    ...
    def predict_action(self,sample,features,weights,total_loss=None,verbose=False):
        ...
        # print(action_results["action_reg_possibilities"])
