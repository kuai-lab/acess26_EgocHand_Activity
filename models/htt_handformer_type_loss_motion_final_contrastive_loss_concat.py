...
from HandFormer.HandFormer.models.hf_pose_motion_FINAL import HF_Pose       
...
from HandFormer.HandFormer.models.transformer_bimodal_bidirection import Bimodal_TF
import numpy as np
import torch
import cv2
...
class TemporalNetHandFormer(nn.Module):
    def __init__(self, 
        ...
        ## Hand Type module (HTT style)  => 06/22
        self.num_handtypes=dataset_info.num_handtypes
        self.hand_type_classification = HandTypeClassificationBranch(num_types=self.num_handtypes, hand_feature_dim=512)
        self.clip_model, _ = clip.load("ViT-B/32", device=self.device)
        self.hand_pred_label_txt = [
            'None', 'Quadpod', 'small Diameter', 'Medium Diameter', 'Thumb up', 'Thumb-Middle Grip',
            'Tip Pinch', 'Disk Grip', 'Dynamic Tripod', 'Fixed Hook', 'Fist', 'Large Diameter',
            'parallel Extension', 'Thumb-2 Finger', 'Writing Tripod', 'Tripod', 'Hand Clench',
            'Pincer Grip', 'Open Hand', 'Stirring', 'Spray-Trigger Grip', 'Index Finger Flexion',
            'Thumb Tucked', 'Extended Index Curl', 'Relaxing Hand', 'Dynamic Flatten', 'Dynamic Pinch',
            'Index Finger', 'Full Rotation', 'Dynamic Parallel Extension', 'Dynamic Lateral Pinch',
            'Poking', 'Middle Rotation', 'Index Rotation', 'Adduction Grip', 'Dynamic Diameter',
            'Palmar', 'Hammering', 'Extension Type'
        ]
        ...
        # Bidirectional classifier (06/21)
        self.classifier_bidirectional = nn.Linear(embedding_dim_final, 2)  # 0 or 1 (bidirectional class)
        self.clip_feat_proj = nn.Linear(512, 256)  # 512 -> 256 projection
    ...
    def forward(self, batch_flatten, epoch=0, train=True, verbose=False, return_atten_map=False):
        ...
        window_size = 15
        rgb_feat_flat, _ = self.meshregnet(rgb_img)  # [B*T, D] resnet 통과 [B*T, 512]=[8*120, 512] (ResNet 마지막 pooling layer output)
        rgb_feat = rgb_feat_flat.view(B, T, -1)      # [B, 128, D]

        valid_T = (T // window_size) * window_size        #  120(15 배수)
        ...
        ### ======================================================================= ###
        # frame 단위 hand type classifier
        # import pdb; pdb.set_trace()
        handtype_outs = self.hand_type_classification(rgb_feat_flat)  # dict
        handtype_logits = handtype_outs["reg_outs"]                   # [B*T, num_handtypes]
        probs_verb = torch.softmax(handtype_logits, dim=-1)    # [B*T, num_handtypes]
        pred_verb = torch.argmax(probs_verb, dim=-1)           # [B*T]
        
        # CLIP prior 생성 (frame 단위)
        clip_feats = self.hlabel_features[pred_verb].float()   # [B*T, D_clip] = [960, 512] => clip embedding space
        clip_feats = self.clip_feat_proj(clip_feats)          # [B*T, 256]
        clip_feats_proj = F.normalize(clip_feats, dim=-1)
        clip_feats_proj = clip_feats_proj.view(B, T, -1)  # [B, T, D] = [8,120,512]
        clip_micro = clip_feats_proj.view(B, 8, 15, -1).mean(dim=2)  # [B, 8, D ]
        ...
        # Trajectory Encoder 통과시킴 (Bidirectional Motion-vector)  => Pose Token + Rotation-Prior Token 컨캣된 상황
        pose_token , rotation_prior_token = self.pose_net(pose_3d)                  # [B, K(num_mactions), D]=[2,8,256]
        # import pdb; pdb.set_trace()
        # rotation_prior_token: [B, T, D]
        # pooling 해서 [B, D] or [B*T, D] 형태로 contrastive loss 입력에 넣을 수 있음

        pose_feat = self.pose_proj(pose_token)               # [B, K(num_maction), D_in] → [B, T, D_out]
        ...
        # ===== Rotation Prior 최적화 =====
        # pose_feat, rgb_feat, rotation_prior_token -> transformer 입력으로 독립 token화
        if self.MIB_block: # Pass mixed features to transformer -- along with residual connections from pose and rgb features
            # maction_feat_for_tf = fused_feat + torch.cat([pose_feat ,rgb_feat], dim=-1) + torch.cat([pose_feat,rotation_prior_token], dim=-1) # [B, T_m, D]
            # import pdb;pdb.set_trace()
            maction_feat_for_tf = fused_feat + \
                      torch.cat([pose_feat, rgb_feat], dim=-1) + \
                      torch.cat([pose_feat, rotation_prior_token], dim=-1) + \
                      torch.cat([rgb_feat, clip_micro], dim=-1)     # 바꿈(06/24)
        else:
            maction_feat_for_tf =  torch.cat([pose_feat, rgb_feat, concat_feat_with_rot], dim=-1)# Ignored mixed features (no PoseRGB feat)
       
       # Temporal transformer output (Sumary_motion 추가 06/21)
        if return_atten_map:
            summary, summary_v, summary_n, summary_m, atten_map = self.mib_transformer(maction_feat_for_tf, return_atten_map)
        else:   
            summary, summary_v, summary_n, summary_m = self.mib_transformer(maction_feat_for_tf)                
        ...
        out_bidirection = self.classifier_bidirectional(summary_m)   # Bi-directional loss
        ...
        # (4) Bidirectonal Loss
        weights_bidirectional_loss = torch.ones_like(batch_flatten['not_padding'].cuda().float()[0::self.ntokens_action])
        bidirectional_results, total_loss, bidirectional_losses = self.predict_bidirectional(
            sample=batch_flatten,
            features=out_bidirection,
            weights=weights_bidirectional_loss,
            total_loss=total_loss
        )
        losses.update(bidirectional_losses)

        # bidirectional_label: [B], expand to match [B*T]
        # rotation_prior_token: [B, T, D]
        # DataParallel 때문에 GT도 local batch 단위로 slice 필요
        ...
        # import pdb; pdb.set_trace()
        if return_atten_map:
            return out, out_verb, out_noun, out_bidirection, l1_loss.unsqueeze(0), atten_map
        else:
            return out, out_verb, out_noun, out_bidirection, losses
...
###----------------------------------------------------------------\
    ...
    ## Bidirectional Loss 추가 *06/21
    def predict_bidirectional(self, sample, features, weights, total_loss=None, verbose=True):
        """
        양방향 액션에 대한 CrossEntropyLoss 계산
        - GT label == -1 → loss 계산에서 제외 (마스킹)
        - GT label == 0 또는 1 → loss 계산에 포함
        """
        logits = features  # [B, 2]
        ...
        # GT 가져오기
        # import pdb;pdb.set_trace()
        gt_bidirectional_labels = sample['bidirectional_label'].cuda()  # [B]
        device = features.device
        B = pred_labels.shape[0]
        # 유효한 샘플 마스크 (GT != -1)\
        gt_bidirectional_labels = gt_bidirectional_labels.view(B, -1)[:, 0]  # B만 남기기
        valid_mask = (gt_bidirectional_labels != -1)
        weights = weights.view(B, -1).mean(dim=1)

        if valid_mask.sum() == 0:
            # 유효한 샘플이 없다면 loss=0
            bidirectional_loss = torch.tensor(0.0, device=device)
        else:
            valid_logits = logits[valid_mask]
            valid_gt = gt_bidirectional_labels[valid_mask]
            valid_weights = weights[valid_mask]

            # CrossEntropyLoss
            ce_loss = F.cross_entropy(valid_logits, valid_gt, reduction='none')  # [N_valid]
            ce_loss = ce_loss * valid_weights
            bidirectional_loss = ce_loss.sum() / (valid_weights.sum() + 1e-5)

        # total_loss update
        if total_loss is None:
            total_loss = self.lambda_action_loss * bidirectional_loss
        else:
            total_loss += self.lambda_action_loss * bidirectional_loss

        # 결과 dict
        bidirectional_results = {
            "bidirectional_gt_labels": gt_bidirectional_labels,
            "bidirectional_pred_labels": pred_labels,
            "bidirectional_reg_possibilities": probs
        }

        bidirectional_losses = {"bidirectional_loss": bidirectional_loss}

        # Debug 출력 (선택)
        if verbose:
            print(f"[Bidirectional Prediction] valid count: {valid_mask.sum().item()}")
            for i in range(min(5, logits.size(0))):
                print(f"  Sample {i}: GT={gt_bidirectional_labels[i].item()} Pred={pred_labels[i].item()}")

        return bidirectional_results, total_loss, bidirectional_losses
    ...
    def predict_handtype(self, sample, features, weights, total_loss=None, verbose=False):
        ...
        hlabel_gts_right = sample['hand_label_right'].cuda().view(-1)  # [B*T]
        ...
        hlabel_loss = hlabel_loss * weights
