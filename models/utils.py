...
def recover_3d_proj_pinhole(camintr, est_scale, est_trans,off_z=0.4, input_res=(128, 128), verbose=False):
    ...
    batch_size = camintr.shape[0]
    focal = camintr[:, :1, :1]   # (1,1,1) or (B,1,1)

    if focal.shape[0] != batch_size:
        focal = focal.repeat(batch_size, 1, 1)  # (B,1,1)

    focal = focal.view(batch_size, 1, 1)         # (B,1,1) (정상)
...
class To25DBranch(nn.Module):
    ...
    def __init__(self, trans_factor=1, scale_factor=1):
        ...
        self.cam_intr[:2] = self.cam_intr[:2] * 1/4
    ...
    def forward(self, sample, scaletrans, verbose=False):        
        ...
        thermal_img = sample[TransQueries.IMAGE][1] if isinstance(sample[TransQueries.IMAGE], (tuple, list)) else sample[TransQueries.IMAGE]
        height, width = tuple(thermal_img.shape[2:])  # ✅ 안전하게 처리
        # camintr = sample[TransQueries.CAMINTR].cuda() 
        camintr = torch.tensor(self.cam_intr, dtype=torch.float32).clone().cuda()

        # camintr가 (3,3) 이면 batch_size만큼 복제
        if camintr.dim() == 2:
            batch_size = scaletrans.shape[0]  # flatten_hpose의 batch size
            camintr = camintr.unsqueeze(0).expand(batch_size, -1, -1)  # (B,3,3)
...
def compute_hand_loss_2d(
    ...
    """
    2D 관절 좌표에 대한 손실만 계산합니다.
    
    Args:
        est2d: (B, J, 2) 예측된 2D 관절 좌표
        gt2d:  (B, J, 2) GT 2D 관절 좌표
        weights: (B*J,) 또는 (B, J) 형태의 binary mask (padding=0, valid=1)
        pose_loss: loss function (e.g. nn.MSELoss) – must support reduction='none'
        verbose: 디버깅용 출력 여부 (미사용)
    
    Returns:
        hand_losses: {"recov_joints2d": scalar} 형태의 딕셔너리
    """
    hand_losses = {}
    # padding이 모두 0일 때 나누기 0 방지
    sum_w = torch.sum(weights)
    sum_weights = torch.where(sum_w > 0, sum_w, torch.tensor([1.], device=weights.device))[0]

    if est2d is not None:
        # per-element loss → (B, J, 2)
        loss2d = pose_loss(est2d, gt2d, reduction='none')
        # flatten joint·coord 축 → (B, J*2, 1)
        loss2d = loss2d.view(loss2d.shape[0], -1, 1)
        # 여기 차원 맞나 -> 원본 코드가 이런식이긴한데
        loss2d = torch.bmm(loss2d, weights.view(-1,1,1))
        # 관절 수·유효 포인트 수로 나누어 평균
        hand_losses["recov_joints2d"] = torch.sum(loss2d) / (loss2d.shape[1] * sum_weights)
