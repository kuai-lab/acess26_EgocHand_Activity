...
class TemporalNet(torch.nn.Module):
    ...
    def compute_contrastive_loss_jw(self, features, labels, temperature=0.07):
        # features: [N, D]
        # labels: [N]
        norm_feat = F.normalize(features, dim=-1)
        logits = torch.matmul(norm_feat, norm_feat.T) / temperature  # [N, N]
        ...
        # Exclude self-comparison (그니까 이게 flatten된거에서 0이랑 1 동시에 존재하는 경우에서만 비교하게 해버려서 남는샘플이 없어지는 경우도 있었음)
